...
const DeleteCatButton = ({ CatIds, onDeleteComplete ,onSelectCategory }) => {
  ...
  useEffect(() => {
    ...
    if (data && data.SubCategories ) {
      const cats = data.SubCategories.filter(cat => CatIds.includes(cat.id));
      setShowButton(CatIds && CatIds.length > 0 && cats.length > 0);
      setCatssToDelete(cats);
    }
    if (queryError) {
      console.error('Error fetching SubCategories:', queryError.message);
    }
  }, [data, queryError, CatIds]);


  const [deleteCats] = useMutation(DELETE_Cat, {
    update(cache, { data: { deletSubCategory } }) {
      try {
        const existingData = cache.readQuery({ query: GET_Cat });
        if (existingData && existingData.SubCategories) {
          const remainingCats = existingData.SubCategories.filter(
            (Cat) => !deletSubCategory.some((deletedCat) => deletedCat.id === Cat.id)
          );
          cache.writeQuery({
            query: GET_Cat,
            data: { SubCategories: remainingCats },
          });
        }
      } catch (error) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const existingCategories = data.SubCategories.map(category => category.id);
  ...
  return (
    <div className="flex flex-col items-center">
      {showButton && (
        ...
        >
          {loading ? 'Deleting...' : 'Delete Selected Sub'}
        </button>
      )}
      {error && (
